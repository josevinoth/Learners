...
from ..forms import EmpeditForm,UserexteditForm
...
def user_add(request,user_id=0):
    ...
    if request.method == "GET":
        ...
        return render(request, "learners_app/signup_page_crud.html", {'form': form,'user_ext_edit_form': user_ext_edit_form,'first_name': first_name})
    else:
        if user_id == 0:
            ...
            return redirect('/learners/user_list')
...
def user_list(request):
    ...
    return render(request,"learners_app/employee_list.html",context)

@login_required(login_url='login_page')
def user_list_temp(request):
    first_name = request.session.get('first_name')
    context = {'user_list_temp' : User.objects.filter(is_active=False),'first_name': first_name,}
    return render(request,"learners_app/employee_list_temp.html",context)
#Delete User
@login_required(login_url='login_page')
